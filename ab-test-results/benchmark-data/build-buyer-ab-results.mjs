...
const format = (key, value) =>
  key === "downloads" ? `${value.toFixed(1)} KB` : key === "downloads-count" ? `${Math.round(value)}` : time(value);
const text = (x, y, value, size = 16, attrs = "") =>
  `<text x="${x}" y="${y}" font-size="${size}" ${attrs.includes("fill=") ? "" : `fill="${ink}"`} ${attrs}>${xml(value)}</text>`;
...
function chart(metricDefinitions, title, subtitle, filename) {
  ...
  metricDefinitions.forEach(({ key, heading }, index) => {
    ...
    const max =
      Math.max(
        ...data.results.flatMap((result) => [result.metrics[key].controlMedian, result.metrics[key].experimentMedian]),
      ) * 1.32;
...
function distributionByCache() {
  ...
  const sections = [
    {
      cache: "Empty cache",
      title: "Cold visits · seconds",
      max: 18000,
      ticks: [0, 4000, 8000, 12000, 16000, 18000],
      headingY: 190,
      gridTop: 218,
      gridBottom: 445,
      rows: [250, 355],
      tick: (value) => `${value / 1000}s`,
    },
    {
      cache: "Prepopulated cache",
      title: "Warm visits · milliseconds",
      max: 1000,
  ...
  for (const section of sections) {
    ...
    assert.ok(
      matching.every(
        (result) =>
          Math.max(...result.metrics.FCP.samples.control, ...result.metrics.FCP.samples.experiment) <= section.max,
      ),
    );
    ...
    for (const [index, result] of matching.entries()) {
      ...
      for (const [side, offset, color] of [
        ...
        const low = Math.min(...values),
          ...
          median = result.metrics.FCP[`${side}Median`];
        const y = top + offset;
        parts.push(
  ...
  parts.push(
    text(
      48,
      817,
      "Line = minimum to maximum; circle = median. Different cold and warm scales; neither shows paired intervals.",
      15,
      `fill="${muted}"`,
    ),
  );
...
const outputs = [
  ...
  chart(
    [
      { key: "TTFB", heading: "Browser-observed TTFB" },
