...
let metadata;
if (snapshotOnly) {
  const replay = await readFile(output, "utf8");
  ...
  assert.deepEqual(metadata.frameCounts, {
    ...
    experiment: sides.experiment.frames.length,
  });
  durationMs = metadata.durationMs;
  sourceHashes.push(...metadata.sourceHashes);
} else {
  ...
  for (const side of ["control", "experiment"]) {
    ...
    assert.match(report.environment.networkUserAgent, /Mobile/u);
    assert.match(report.configSettings.emulatedUserAgent, /Mobile/u);
  ...
  durationMs =
    Math.ceil(Math.max(...Object.values(sides).flatMap((side) => side.frames.map((frame) => frame.timeMs))) / 1000) *
    1000;
  metadata = {
    runId: perf.runId,
    caseId,
...
function imageDocument(title, description, body, height) {
  return `${[
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 ${
      height
    }" role="img" aria-labelledby="title description" font-family="Arial,Helvetica,sans-serif">`,
    `<title id="title">${escapeXml(title)}</title>`,
    `<desc id="description">${escapeXml(description)}</desc>`,
    `<metadata>${escapeXml(
      JSON.stringify({ runId: metadata.runId, caseId, diagnosticStage: metadata.diagnosticStage, sourceHashes }),
    )}</metadata>`,
...
for (const [index, timeMs] of [0, 2000, 6000, 10000].entries()) {
  ...
  preview.push(
    `<text x="${column}" y="136" fill="#162334" font-size="21" font-weight="700">${timeMs / 1000} seconds</text>`,
  );
  for (const [side, offset, color, label] of [
    ...
    preview.push(
      `<image href="${frame.image}" x="${x}" y="162" width="138" height="245" preserveAspectRatio="xMidYMin meet"/>`,
    );
    preview.push(`<text x="${x}" y="437" fill="${color}" font-size="17" font-weight="700">${label}</text>`);
  }
}
preview.push(
  `<text x="44" y="493" fill="#162334" font-size="19" font-weight="700">First paint in this capture: Inertia ${(
    sides.control.fcpMs / 1000
...
for (const [side, y, color, label] of [
  ...
  timeline.push(
    `<rect x="${axisStart}" y="${y - 12}" width="${fcpX - axisStart}" height="24" rx="5" fill="${
      color
    }" opacity="0.22"/>`,
  );
  ...
  timeline.push(
    `<text x="${labelX}" y="${y - 20}" text-anchor="${anchor}" fill="${color}" font-size="18" font-weight="700">FCP ${(
      fcp / 1000
    ).toFixed(2)} s</text>`,
  );
  timeline.push(
    `<text x="${labelX}" y="${y + 49}" text-anchor="${anchor}" fill="#4d5a66" font-size="16">LCP ${(lcp / 1000).toFixed(
      2,
    )} s</text>`,
  );
}
...
if (!snapshotOnly)
  assert.equal(
    sha256(await readFile(join(caseDir, "artifacts/timeline_comparison.html"))),
    metadata.originalTimeline.sha256,
    "Original timeline changed during generation",
  );
process.stdout.write(
  `Regenerated replay graphs from ${snapshotOnly ? "saved frames" : "source traces"}: ${metadata.frameCounts.control}/${metadata.frameCounts.experiment} frames, ${durationMs} ms`,
);
