...
The required final evidence is preserved in two independent, self-contained
sets:

- `final/inertia-baseline-vs-rorp`: exact Inertia baseline versus the final
  RORP implementation.
- `final/same-commit-flag-off-vs-on`: the same final source on both sides, with
  only the seller-scoped feature state changed.

Each directory contains a summary, its self-contained HTML report, and the raw
per-case performance and visual-regression results.
