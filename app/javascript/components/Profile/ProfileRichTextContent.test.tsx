...
it("detects nested nodes that require client enhancement", () => {
  expect(
    profileRichTextNeedsClientEnhancement({
      type: "doc",
      content: [{ type: "paragraph", content: [{ type: "text", text: "Static creator story" }] }],
    }),
  ).toBe(false);
  expect(
    profileRichTextNeedsClientEnhancement({
      type: "doc",
      content: [{ type: "paragraph", content: [{ type: "raw" }] }],
    }),
  ).toBe(true);
});
