...
describe("ProductSellerAndRatings", () => {
  it("hides a seller-only byline when the surrounding profile already shows it", () => {
    render(<ProductSellerAndRatings content={content} hideSellerByline />);

    expect(screen.queryByRole("link", { name: "Seller" })).toBeNull();
  });

  it("retains seller and collaborator attribution when hiding a duplicate seller-only byline", () => {
    render(
      <ProductSellerAndRatings
        content={{
          ...content,
          collaborating_user: {
            ...
            profile_url: "https://example.com/collaborator",
            is_verified: false,
          },
        }}
        hideSellerByline
    ...
    expect(screen.getByRole("link", { name: "Seller" })).toBeTruthy();
    expect(screen.getByRole("link", { name: "Collaborator" })).toBeTruthy();
  });
});
