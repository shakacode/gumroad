...
const waitForStableSize = async (page: Page, selector: string) => {
  await page.locator(selector).evaluate(async (element: HTMLElement) => {
    ...
    if (stableFrames < 3) throw new Error("Captured component size did not settle within 60 animation frames");
  });
};
...
abTest(
  "Product Page - Discover layout warm landing",
  {
    ...
    config: warmPerfConfig(warmCurrentPage((page) => waitForProduct(page, standardProduct))),
  },
  async ({ page }) => waitForProduct(page, standardProduct),
);
...
const completeProfileCheckout = async ({ page, annotate }: TestFnContext) => {
  ...
  // Link initializes after the card field; auditing earlier races its iframe accessibility state.
  await checkout
    .frameLocator('iframe[name^="cardButton"]')
    .getByRole("button", { name: "Autofill with Link", exact: true })
    .waitFor({ state: "visible" });
  await page.waitForFunction(() => {
    const frame = document.querySelector<HTMLIFrameElement>('[data-checkout-scope] iframe[name^="cardButton"]');
    return frame?.getAttribute("tabindex") === "-1";
  });
