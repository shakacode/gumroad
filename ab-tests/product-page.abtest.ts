...
const standardProduct: ProductFixture = {
  ...
  experimentUrl: `http://luisfurushio.experim.localhost:${EXPERIMENT_PORT}/l/bgfjk?layout=discover&recommended_by=search`,
  layout: "discover",
};
...
const sellerProfileProduct: ProductFixture = {
  ...
  experimentUrl: `http://luisfurushio.experim.localhost:${EXPERIMENT_PORT}/l/bgfjk?layout=profile&recommended_by=search`,
  layout: "profile",
};
...
const prepareProductLandingNavigation: BeforeNavigateHook = async (context) => {
  await prepareShakaPerfNavigation(context);
  await installRequestBlocking(context.context, ["js.stripe.com/v3"]);
};
...
const warmCurrentProduct =
  ...
  async (context) => {
    await prepareProductLandingNavigation(context);
...
abTest(
  "Product Page - Discover layout cold landing",
  {
    ...
    config: { shared: { beforeNavigate: prepareProductLandingNavigation } },
    ...landingCoverage,
  },
