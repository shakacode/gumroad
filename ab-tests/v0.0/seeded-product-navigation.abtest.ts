...
abTest(
  ...
  async ({ page, annotate, isControl }) => {
    await page.locator(isControl ? "#app[data-page]" : "#native-product-rsc-root").waitFor({
      state: "attached",
      timeout: PAGE_READY_TIMEOUT_MS,
    });
    await page.getByRole("heading", { level: 1, name: product.name, exact: true }).waitFor({
      state: "visible",
      timeout: PAGE_READY_TIMEOUT_MS,
    });
    ...
    await Promise.all([
      ...
      creatorLink.click({ timeout: PAGE_READY_TIMEOUT_MS }),
    ]);
