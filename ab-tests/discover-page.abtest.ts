...
const waitForCategory = async (
  page: Page,
  name: string = CATEGORY_NAME,
  expectedCardCount: number = DISCOVER_CARD_COUNTS.programming,
) => {
  await page
    .getByRole("link", { name, exact: true })
    .and(page.locator('[aria-current="page"]'))
    .waitFor({ state: "visible" });
  await waitForMarketplace(page, expectedCardCount);
};
...
for (const colorScheme of ["light", "dark"] as const) {
  abTest(
    `Discover Page - Programming category ${colorScheme} cold landing`,
    {
      startingPath: CATEGORY_PATH,
      testTypes: colorScheme === "light" ? ["perf", "visreg", "accessibility", "audit"] : ["visreg", "accessibility"],
