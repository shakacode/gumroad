...
RSpec.describe "benchmark Rails environment" do
  CONFIG_RUNNER = <<~'RUBY'
    ...
    payload = {
      ...
      dynamodb_endpoint: EmailEngagementDynamoStore.client.config.endpoint.to_s,
      dynamodb_table_prefix: EmailEngagementDynamoStore.table_prefix,
      dynamodb_table_name: EmailEngagementDynamoStore.table_name,
      domain: DOMAIN,
      protocol: PROTOCOL,
  ...
  def run_environment(environment, runner, extra_env = {})
    env = {
      ...
      "AWS_ACCESS_KEY_ID" => "benchmark-spec",
      "AWS_SECRET_ACCESS_KEY" => "benchmark-spec",
      "DYNAMODB_ENDPOINT" => "http://127.0.0.1:8000",
      "DYNAMODB_TABLE_PREFIX" => "benchmark_",
  ...
  it "uses disposable local database, DynamoDB, and MinIO configuration" do
    expect(@benchmark_config).to include(
    ...
    expect(@benchmark_config[:dynamodb_endpoint]).to eq("http://127.0.0.1:8000")
    expect(@benchmark_config[:dynamodb_table_prefix]).to eq("benchmark_")
    expect(@benchmark_config[:dynamodb_table_name]).to eq("benchmark_email_engagement")
