...
RSpec.describe "Control Plane image and secret bootstrap" do
  ...
  it "prepares only fixed backing-service secrets and refuses implicit rotation" do
    ...
    expect(bootstrap).to include('R2_BUCKET="shaka-perf-demo-storage"')
    expect(bootstrap).to include("S3_ENDPOINT", "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY")
    expect(bootstrap).to match(/--entry "bucket=\$\{R2_BUCKET\}" \\\n+    >\/dev\/null/)
    expect(bootstrap).not_to match(/minio/i)
    expect(bootstrap).not_to include("S3_BUCKET")
