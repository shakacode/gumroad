...
describe Discover::CanonicalUrlPresenter do
  ...
  describe "#canonical_url" do
    it "returns the /discover url when no valid search parameters are present" do
      params = ActionController::Parameters.new({})
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover")
    ...
    it "returns the url with parameters" do
      ...
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?query=product")
      ...
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?tags=3d+model")
    end
    ...
    it "returns the url with sorted parameters and values" do
      ...
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?query=product&rating=1&sort=featured")

      params = ActionController::Parameters.new({ max_price: 1, tags: ["tagb", "taga"], sort: "hot_and_new" })
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?max_price=1&sort=hot_and_new&tags=taga%2Ctagb")
    ...
    it "ignores empty parameters" do
      ...
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?max_price=0&query=product")
    end
    ...
    it "correctly formats array parameters" do
      ...
      expect(described_class.canonical_url(params)).to eq("#{discover_domain_with_protocol}/discover?filetypes=mp3%2Czip&tags=tag1%2Ctag2")
    end
  end
end
